function [revenue, cash, investment] = arbitrary_3Market_combinedStrategy(inventory, C5_matrix, igxe_matrix, bitskin_matrix, tradingQuantPerHour, strategy)
...
function printNoiseTransaction(BS_F, xx, M_F, BS_Amount, possib)
    ...
    if BS_F == "buy"
        ...
        fprintf('%c%c \n', 8, 8); %删掉2个换行符
    elseif BS_F == "sell"
        fprintf('--噪点--  噪点售出：编号 %d ,市场 %s ,金额 %d   现金：%d   概率：%3.5f', xx, MM_F, BS_Amount, cash, possib);
        fprintf('当前库存：');
        disp(items{xx});
        fprintf('%c%c \n', 8, 8); %删掉2个换行符
    end
...
function itemsValueCheck()
    ...
    fprintf('资产总价值：%d\n',totalValue+cash);
    fprintf('初始投入：%d\n',investment);
    revenue = totalValue + cash - investment;
end
...
function transcationPending(xx, index)
    ...
    %TODO 如果卖出，处理buyFlag
    possib = possibility(xx, timeFlag, sellFlag);
