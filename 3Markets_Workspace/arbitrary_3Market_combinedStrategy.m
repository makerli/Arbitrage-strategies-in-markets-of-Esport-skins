function [cash, investment] = arbitrary_3Market_combinedStrategy(inventory, C5_matrix, igxe_matrix, bitskin_matrix, tradingQuantPerHour, strategy)

%策略：original 和 noise 两策略同时进行
...
function printNoiseTransaction(BS_F, xx, M_F, BS_Amount)
    if class(M_F) == 'string'
        MM_F = M_F;
    else
        if M_F == IGXE
    ...
    if BS_F == "buy"
        fprintf('--噪点--  噪点购入：编号 %d ,市场 %s ,金额 %d   现金：%d ', xx, MM_F, BS_Amount, cash);
        fprintf('当前库存：');
        disp(items{xx});
        fprintf('%c%c \n', 8, 8); %删掉2个换行符
    elseif BS_F == "sell"
        fprintf('--噪点--  噪点售出：编号 %d ,市场 %s ,金额 %d   现金：%d ', xx, MM_F, BS_Amount, cash);
        fprintf('当前库存：');
        disp(items{xx});
        fprintf('%c%c \n', 8, 8); %删掉2个换行符
    end
...
function transcationPending(xx, index)
    ...
    %TODO 如果卖出，处理buyFlag
    if rand(1) < tradingQuantPerHour(xx)          %TODO TODO TODO 需要换成估计概率的函数
...
function noiseTransactionPending(timeNow, xx, noise_F)
    if noiseRec(xx) == 1 && noisePending(xx) == 0
        ...
        noisePending(xx) = 1;
        printNoiseTransaction("buy", xx, noise_F, noisePriceTemp(xx));
    end
    if feasibleMarketToSellOn(xx, timeNow) == C5
        if C5_matrix(xx, timeNow) > noisePriceTemp(xx) * 1.06     %如果满足卖出的差价条件
            if rand(1) < tradingQuantPerHour(xx)
                if checkFeasibility(xx) ~= -1
                    revenueTemp = sellOnNoiseStrategy(C5, xx, checkFeasibility(xx));
                    noisePending(xx) = 0;
                    printNoiseTransaction("sell", xx, "C5", revenueTemp);
                end
            end
        end
    elseif feasibleMarketToSellOn(xx, timeNow) == IGXE
        if igxe_matrix(xx, timeNow) > noisePriceTemp(xx) * 1.06
            if rand(1) < tradingQuantPerHour(xx)
                if checkFeasibility(xx) ~= -1
                    revenueTemp = sellOnNoiseStrategy(IGXE, xx, checkFeasibility(xx));
                    noisePending(xx) = 0;
                    printNoiseTransaction("sell", xx, "IGXE", revenueTemp);
                end
            end
        end
    else
        if bitskin_matrix(xx, timeNow) > noisePriceTemp(xx) * 1.06
            if rand(1) < tradingQuantPerHour(xx)
                if checkFeasibility(xx) ~= -1
                    revenueTemp = sellOnNoiseStrategy(BITSKIN, xx, checkFeasibility(xx));
                    noisePending(xx) = 0;
                    printNoiseTransaction("sell", xx, "BITSKIN", revenueTemp);
                end
            end
        end
    end
...
for i = 1 : t       
    ...
    for x = 1 : size(C5_matrix,1)
        %%% noiseStrategy Start —— noiseStrategy Start —— noiseStrategy Start
...
%                     noisePriceTemp(x) = bitskin_matrix(x,i);
        ...
        %%% noiseStrategy End —— noiseStrategy End —— noiseStrategy End
        if breakFlag == 1
            breakFlag = 0;
            break;
        end
