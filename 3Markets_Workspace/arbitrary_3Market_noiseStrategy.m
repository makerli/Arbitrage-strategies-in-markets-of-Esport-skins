function [cash, investment, noiseMark_C5, noiseMark_IGXE, noiseMark_BITSKIN] = arbitrary_3Market_noiseStrategy(inventory, C5_matrix, igxe_matrix, bitskin_matrix, tradingQuantPerHour, strategy)
...
function printNoiseTransaction(BS_F, xx, M_F, BS_Amount)
    ...
    if BS_F == "buy"
        fprintf('噪点购入：编号 %d ,市场 %s ,金额 %d   ', xx, MM_F, BS_Amount);
        fprintf('当前库存：');
        disp(items{xx});
        fprintf('%c%c \n', 8, 8); %删掉2个换行符
    elseif BS_F == "sell"
        fprintf('噪点售出：编号 %d ,市场 %s ,金额 %d   ', xx, MM_F, BS_Amount);
        fprintf('当前库存：');
        disp(items{xx});
        fprintf('%c%c \n', 8, 8); %删掉2个换行符
    end
